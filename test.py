...
from ldm.data.dc_dataset import Dataset
...
#将一个可迭代对象按照指定的大小进行分块处理
def chunk(it, size):
    it = iter(it)
    return iter(lambda: tuple(islice(it, size)), ())    #使用islice函数从迭代器it中获取size个元素，返回一个迭代器,再用tuple()转换为元组,使用iter函数构建一个迭代器,第二个参数是迭代器的哨兵值，当匿名函数返回这个哨兵值时，迭代器将停止迭代


#返回一个 torchvision.transforms.Compose 对象，将 transform_list 中的图像转换操作按顺序组合起来(转换为张量并归一化)
...
#加载代替图像，加载替代图像成功，则返回替代图像；否则，返回原始图像
...
def main():
    ...
    dataset = Dataset(opt.dataroot, phase='test', order='unpaired')
